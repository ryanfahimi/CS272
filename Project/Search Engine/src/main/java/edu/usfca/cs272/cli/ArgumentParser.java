...
public class ArgumentParser {
	...
	public static boolean isFlag(String arg) {
		return arg != null && arg.startsWith("-") && arg.length() >= 2 && !Character.isDigit(arg.codePointAt(1))
	...
	/**
	 * Returns the value the specified flag is mapped as a positive int value, or
	 * the backup value if unable to retrieve this mapping (including being unable
	 * to convert the value to an int, if no value exists, or if the int is not
	 * positive).
	 *
	 * @param flag the flag whose associated value will be returned
	 * @param backup the backup value to return if there is no valid mapping
	...
	public int getPositiveInteger(String flag, int backup) {
		int number = getInteger(flag, backup);
		return number <= 0 ? backup : number;
	}

	/**
	 * Returns the value the specified flag is mapped as a positive int value, or 0
	 * if unable to retrieve this mapping (including being unable to convert the
	 * value to an int, if no value exists, or if the int is not positive).
	 *
	 * @param flag the flag whose associated value will be returned
	 * @return the value the specified flag is mapped as an int, or 1 if there is no
	 *   valid mapping
	 *
	 * @see #getInteger(String, int)
	 */
	public int getPositiveInteger(String flag) {
		return getPositiveInteger(flag, 1);
	}
